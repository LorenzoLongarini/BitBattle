...
export async function doMoveMultiplayerService(req: Request, res: Response) {
    ...
    try {
        ...
        let reducedMovesP0 = searchGame[0].dataValues.possible_moves.filter(
            (move: any) => (move.ship >= 1 && move.ship <= 3 && move.owner == emailplayer0)
        );
        let reducedMovesP1 = searchGame[0].dataValues.possible_moves.filter(
            (move: any) => (move.ship >= 1 && move.ship <= 3 && move.owner == emailplayer1)
        );
        let reducedMovesP2 = searchGame[0].dataValues.possible_moves.filter(
        ...
        let currentTurn = isTurn(emailplayer0, emailplayer1, emailplayer2, movesEmail, mod, isPlayingNow0,
            isPlayingNow1, isPlayingNow2, nextMove);
        ...
        if (searchGame[0].dataValues.status !== gameFinishedLabel) {
            ...
            if (isAvailable && !isExecute && currentTurn == jwtPlayerEmail && isHittable) {
                ...
                if (reducedMovesP0.length == reducedMoves0.length && reducedMovesP1.length == reducedMoves1.length) {
                    ...
                    // Se l'utente 0 e l'utente 1 hanno tutte le navi colpite
